        ...
        if isinstance(platform, DefaultPlatformType):
            ...
            if platform == DefaultPlatformType.TWITTER:
                ...
                raise ValueError(f"Invalid platform: {platform}. Only "
                                 ...
                                 "DefaultPlatformType.WEIBO are supported.")
            ...
            platform_type_hint = getattr(platform, "platform_type", None)
            if platform_type_hint == DefaultPlatformType.WEIBO:
                self.platform_type = DefaultPlatformType.WEIBO
            elif (platform.recsys_type == RecsysType.REDDIT
                  or (isinstance(platform.recsys_type, str)
                      and platform.recsys_type == RecsysType.REDDIT.value)):
                self.platform_type = DefaultPlatformType.REDDIT
            else:
                self.platform_type = DefaultPlatformType.TWITTER
        else:
